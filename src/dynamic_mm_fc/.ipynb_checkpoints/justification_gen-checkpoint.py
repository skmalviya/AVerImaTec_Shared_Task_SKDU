...
from dynamic_mm_fc.templates import justi_gen
from dynamic_mm_fc.utils import split_string_by_words
import re
from qwen_vl_utils import process_vision_info
import torch

class Justification_Model:
    def __init__(self, mllm, mllm_name):
        self.verify_mllm=mllm #this is the mllm for justification generation
        self.verify_mllm_name=mllm_name
        self.prompt_temp=justi_gen.just_gen_prompt

    def justi_gen(self, cur_verdict,meta_info,claim_txt, claim_img, evid_context,num_orig_claim_imgs):
        if 'gemini' in self.verify_mllm_name:
            ...
            inputs.append(("\nThe predicted verdict is %s. Please generate your justification (i.e., explanation) for the verdict:") % cur_verdict)
            ...
            with torch.no_grad():
                generated_ids = self.verify_mllm['model'].generate(**inputs, max_new_tokens=1024)
            ...
            evid_texts=[]
            messages[0]['content'].insert(-1,{"type": "text", "text": '\nHere is the evidence: '})
            for i,evid in enumerate(evid_context):
                evid_text=evid['text']
                evid_images=evid['images']
                ...
                """
            messages[0]["content"].append({'type':'text',
                                           'text':'\nHere is the evidence: '+"; ".join(evid_texts)+(("\nThe predicted verdict is %s. Please generate your justification (i.e., explanation) for the verdict:") % cur_verdict)})
            ...
            if self.verify_mllm_name=='llava':
                output = self.verify_mllm['model'].generate(**inputs, max_new_tokens=1024,
