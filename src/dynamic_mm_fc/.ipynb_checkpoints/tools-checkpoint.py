...
def reverse_image_search(ques_txt, img_idx, 
                         ...
                         use_data_store=False, datastore_path=None, test_mode='test'):
    """
    To Be Updated!!
    ...
    for i,image in enumerate(image_lists):
        ...
        if use_data_store:
            ...
            for row in saved_kb:
                all_texts.append(
                    {
                        'scrape_content':' '.join(row['url2text']),
                        'url':row['url']
                    }
                )
        elif os.path.exists(os.path.join(path,'web_det_info/reverse_image_search/scrap',img_name+'.json')):
